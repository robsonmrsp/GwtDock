...
public class DockMenu extends LayoutContainer {
	...
	public void anime(int x, int y) {
		...
		if (horizontalAlign.equals("center"))
			...
			posx = point.x - containerX - offsetWidth + itemWidth * dockItems.size();
		else
			posx = point.x - containerX;
		posy = (int) Math.pow(point.y - containerY - offsetHeight / 2, 2);
		...
		for (int nr = 0; nr < numItens; nr++) {
			...
			LayoutContainer div = dockItems.get(nr).getHtml2();
	...
	private void prepareMouseHandlers() {
		...
		Window.addResizeHandler(new ResizeHandler() {
			...
			public void onResize(ResizeEvent event) {
				int w = DockMenu.this.getWidth();
				// DockMenu.this.getElement().getStyle().setLeft((event.getWidth()
				// - w) / 2, Unit.PX);
